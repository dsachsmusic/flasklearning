...
neighborhoodGoogleURLs = [
    "https://www.google.com/maps/place/University+Hills,+Austin,+TX+78723/@30.3112125,-97.6817029,15z/data=!3m1!4b1!4m6!3m5!1s0x8644c9db48993607:0x3cf5363a3b94b0cc!8m2!3d30.317206!4d-97.6738023!16s%2Fg%2F1tvw59v0?entry=ttu",
...
for url in neighborhoodGoogleURLs:
    place = url.split("place")[1].split("/")[1].split(",")[0].replace("+"," ")
    long_and_lat_split = url.split("@")[1].split(",")
    long_and_lat = []
    long_and_lat = long_and_lat.__add__([long_and_lat_split[0]])
    long_and_lat = long_and_lat.__add__([long_and_lat_split[1]])
    
    
    for item in long_and_lat:
        if float(item) < 0:
            long = item
        else: 
            lat = item

    weather_url = f"https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={long}&appid=bd3987f785f348cc80b9dfb4a1fa4fa4"

    response = requests.get(weather_url)
    response_as_json = json.loads(response.text)

    #print(json.dumps(response_as_json, indent = 4))  
    weather_description = response_as_json["weather"][0]["description"]

    print("The weather in " + place + "is " + weather_description)